(function(global){
  'use strict';
  const E=global.QualificationEngine,C=global.RankingCore,OUT=['W','D','L'],WORD={W:'胜',D:'平',L:'负'},FLIP={W:'L',D:'D',L:'W'};
  ...
  const METHODS={I:'类型Ⅰ · 相互比较不重启',II:'类型Ⅱ · 剩余球队重新比较',III:'类型Ⅲ · 全组数据优先'};
  ...
  function detailContent(detail,cell,input,retry){
    ...
    for(const [status,label] of [['qualified','确定出线'],['pending','出线资格待决胜']]){
      ...
      if(status==='pending')detail.append(make('p',C.PENDING));
    }
    ...
    for(const [status,example] of Object.entries(r.examples)){
      const fixtureText=input.remaining.map(m=>`${m.home} ${example.scores[m.id].join('∶')} ${m.away}`).join('；');
  ...
  function render(host,state,callbacks={}){
    ...
    if(!input.remaining.length){
      ...
      host.append(make('div',`比赛已全部确认。${target}：${TEXT[status]}${group.rank===null?`；第${group.position_from}—${group.position_to}名尚未分出先后`: `，第${group.rank}名`}。${status==='pending'?C.PENDING:''}`,'qualification-empty'));return;
    }
    ...
    const history=make('details',undefined,'qualification-history');history.append(make('summary','已确认赛果'));const hl=make('ul');for(const [a,b,x,y]of input.completed)hl.append(make('li',`${a} ${x}∶${y} ${b}`));history.append(hl);host.append(history);
    let generation=0;
